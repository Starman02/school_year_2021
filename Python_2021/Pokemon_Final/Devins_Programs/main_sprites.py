...
from config import *
# this is a file I copied and pasted from jackson's current working program, because there are some tiny adjustments made to make the programs work together( for this to fully work it would be jacksons program 1.6, but since some modifications are needed to get them to work we use a placeholder file for the program)
# in order for the programs to work with eachother, the host computer needs the images and assets downloaded, specifically the files needed for battleGUI is needed, all sprites and should load perfectly in the main map part of the game.
from loadable_BattleGUI import *
...
class SpriteSheet:  # this class set handles the sprite sheets, and in removing the background
