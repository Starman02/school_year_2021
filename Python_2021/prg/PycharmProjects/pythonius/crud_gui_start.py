...
class CrudGUI:
    ...
    def open_menu(self):
        if self.radio_var.get() == 1:
            ...
            _ = Addcustomer(self.master)
        elif self.radio_var.get() == 3:
            _ = ChangeCustomerEntry(self.master)
...
class LookGUI:
    def __init__(self, master):
        ...
        try:
            input_file = open("customer_file.dat", 'r+b')
    ...
    def search(self):
        try:
            ...
            save_file.close()
            self.info_string.set("error opening file, Creating new one")
...
class Addcustomer:
    ...
    def addition(self):
        try:
            ...
            # Save items to file
            save_file = open("customer_file.dat", "r+b")
            pickle.dump(self.customers, save_file)
            save_file.close()
...
class ChangeCustomerEntry:
    def __init__(self, master):
        ...
        self.add_button = tkinter.Button(self.button_frame, text='Add and Save', command=self.addition_change, width=10)
    ...
    def addition_change(self):
        ...
        try:
            ...
            self.info_string.set("we hit the exception captain")
...
class Delete:
    def __init__(self, master):
        ...
        # when the Toplevel window is closed, it returns focus to the master window
        self.look = tkinter.Toplevel(master)
        self.look.title('Delete Customer Entry')
        ...
        self.remove_button = tkinter.Button(self.bottom_frame, text="Delete", command=self.delete, width=10)
    ...
    def delete(self):
        ...
        name = self.__search_entry.get()
        if name in self.customers:
            # customers is the dictionary
            del self.customers[name]
            self.info_string.set("DELETED")
            save_file = open("customer_file.dat", "r+b")
            pickle.dump(self.customers, save_file)
            save_file.close()

        else:
            self.info_string.set("Error Deleting File")

        # deletes dictionary from file
        # save_file.close()
        # self.info_string.set("data saved and added to database")
